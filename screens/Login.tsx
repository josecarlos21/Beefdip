  ...
  return (
    <div className="min-h-screen bg-background-dark flex flex-col relative overflow-hidden">
      ...
      <div className="relative z-20 flex-1 flex flex-col items-center justify-center px-10 pt-20 animate-reveal">
        <div className="w-24 h-24 bg-primary rounded-card flex items-center justify-center shadow-[0_0_80px_rgba(251,146,60,0.5)] mb-10 rotate-6 border-4 border-white/20 active-scale">
      ...
      <div className="relative z-30 w-full max-w-md glass-pure rounded-t-[4.5rem] p-10 pb-20 space-y-8 animate-reveal" style={{ animationDelay: '0.2s' }}>
        <div className="flex p-2 bg-black/40 rounded-card border border-white/5">
          <button 
            onClick={() => setIsLogin(true)}
            className={`flex-1 py-4 text-[10px] font-black rounded-button transition-all active-scale ${isLogin ? 'bg-primary text-white shadow-xl' : 'text-white/30'}`}
          >
            LOGIN
          </button>
          <button 
        ...
        <form className="space-y-5" onSubmit={(e) => { e.preventDefault(); navigate('/dashboard'); }}>
          <div className="space-y-4">
            <input 
              ...
              className="w-full h-20 px-10 rounded-card bg-white/5 border border-white/10 text-white placeholder:text-white/10 focus:ring-2 focus:ring-primary/50 transition-all font-black outline-none italic uppercase tracking-widest text-[10px]"
            />
            <input 
              type="password" 
          ...
          <button 
            type="submit"
            className="w-full h-22 bg-white text-background-dark font-black text-sm rounded-card shadow-2xl transition-all active-scale flex items-center justify-center gap-4 uppercase tracking-[0.2em] group"
          >
            Iniciando Protocolo
